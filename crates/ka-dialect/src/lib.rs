...
//! model-selector parsing. Phase 0 freezes the data shape; Phase 1 wires real
//! Speakers (anthropic-messages, openai-chat) on top of it.
//!
//! Strictness rule: every table rejects unknown keys, so typos in user
//! overlays fail with the offending key name.
...
/// Per-model behavioral flags — the "~12 flags" start of the dialect system.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Flags {
    /// Wire field carrying reasoning config (empty = none).
    #[serde(default)]
...
/// Parse `vendor/model[:effort]` (also accepts a bare `:effort` suffix with
/// empty vendor for effort-only changes).
...
mod tests {
    ...
    fn embedded_catalog_parses() {
        ...
        let o = c.get("ollama/qwen3-32b").unwrap();
        assert_eq!(o.first_byte_timeout_ms, 0);
        assert!(o.flags.replay_reasoning);
        assert_eq!(o.flags.tool_choice, ToolChoice::PinOrAuto);
    }
    ...
    fn selectors_parse() {
        ...
        let s = parse_selector("ollama/qwen3:32b").unwrap();
        // model containing a colon: rsplit takes the last colon as effort
        assert_eq!(s.model, "qwen3");
        assert_eq!(s.effort.as_deref(), Some("32b"));
