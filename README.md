...
Mouse: the default is **capture** — SGR button reporting makes every click affordance work out of the box: tool rows expand in place, the ▲▼ title arrows and strip popups respond to clicks, and the wheel scrolls at line granularity; ⇧drag still selects natively (every major terminal bypasses capture on Shift), and `↑/↓`/`Ctrl+P/N` recall history. `Ctrl+M` (or `[tui] mouse = "native"`) switches to **native**: nothing is captured, so plain drag selects and pastes with the terminal's own bindings and the wheel scrolls via alternate-scroll (which translates it to ↑/↓) — affordances that need clicks are keyboard-only there (`Ctrl+O`, `Ctrl+↑/↓`); the choice persists.
...
mouse = "capture"         # default: clickable tool rows, jump arrows and
                           # strip buttons, wheel scrolls at line
                           # granularity; ⇧drag selects natively. Set
                           # "native" for plain drag select/paste with
                           # no capture (Ctrl+M toggles + persists)
```
